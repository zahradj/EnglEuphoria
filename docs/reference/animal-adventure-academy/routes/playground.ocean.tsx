...
import { OCEAN_ANIMALS, OCEAN_COLORS, SIZES, article, type OceanAnimal } from "@/lib/lesson/ocean";
import { speak } from "@/lib/lesson/speech";
import { Volume2 } from "lucide-react";
import confetti from "canvas-confetti";
import { PhonicsAnyPhase } from "@/components/lesson/phases/PhonicsAny";
import { MemoryMatchPhase } from "@/components/lesson/phases/MemoryMatch";
import { ChantPhase } from "@/components/lesson/phases/Chant";
import { BrainBreakPhase, TapToCatch, PicturePuzzle, GridMaze } from "@/components/lesson/phases/BrainBreak";
...
import frogImg from "@/assets/phonics/frog.png";
import footImg from "@/assets/phonics/foot.png";
...
export const Route = createFileRoute("/playground/ocean")({
  head: () => ({
    meta: [
      { title: "Ocean · Lesson 3 — Colors, Sizes & Sentences" },
      {
        name: "description",
        content:
          "Pre-A / A1 / A2 Ocean Lesson 3 for ages 4–9: 3 sea animals described with color + size, building on Lessons 1 and 2.",
      },
      { property: "og:title", content: "Ocean · Lesson 3 — Colors, Sizes & Sentences" },
      {
        property: "og:description",
        content: "Ocean animals + color + size: 'It is a small orange fish.'",
      },
    ],
  }),
  component: OceanLesson,
});
...
function OceanLesson() {
  ...
  return (
    ...
    >
      ...
      {idx === 2 && <MemoryMatchPhase items={OCEAN_ANIMALS} pairs={3} onComplete={() => markComplete(2)} />}
      ...
      {idx === 8 && <ChantPhase lines={OCEAN_CHANT} onComplete={() => markComplete(8)} />}
      {idx === 9 && (
        <BrainBreakPhase
          ...
          games={[
            ...
            },
            {
              ...
              render: ({ onSolved }) => (
                <PicturePuzzle
                  image={OCEAN_ANIMALS[0].image}
                  label={OCEAN_ANIMALS[0].name}
                  onSolved={onSolved}
                />
              ),
            },
