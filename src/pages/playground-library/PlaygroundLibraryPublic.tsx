...
/**
 * Public, read-only mirror of PlaygroundLibraryPage — anyone with the link
 * can browse the curriculum, no account needed. Unlike the admin page, this
 * shows every published Playground lesson (not just the logged-in creator's
 * own rows), and "Coming Soon" cards are inert instead of jumping into the
 * (protected) authoring tool.
 */
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .limit(500);
      if (error) {
        console.error('Public Playground Library fetch error', error);
        setRows([]);
      } else {
      ...
      <header className="sticky top-0 z-20 border-b-4 border-orange-200/70 bg-[#FFF8E7]/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <button
            ...
            className="rounded-full px-4 py-2 text-sm font-black text-white shadow transition hover:scale-105 active:scale-95"
            style={{ background: 'linear-gradient(135deg,#FE6A2F,#FEBE4C)' }}
          >
            Start Learning →
          </button>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        {!LEVELS.find((l) => l.code === activeLevel)?.curriculum ? (
          <div className="rounded-3xl bg-white p-12 text-center shadow-xl">
            ...
            <p className="mt-1 text-sm font-medium text-orange-700/70">No curriculum here yet — check back soon.</p>
          </div>
        ) : loading ? (
          ...
          <div className="rounded-3xl bg-white p-10 text-center shadow-xl">
            <p className="text-lg font-bold text-orange-800">No Playground units published yet.</p>
          </div>
        ) : (
                      ...
                      {u.lessons.map((l) => {
                        ...
                        return (
                          <button
                            ...
                            disabled={!ready}
                            className={`flex flex-col items-start gap-1 rounded-2xl border-2 p-4 text-left shadow-sm transition ${
                              ready
                                ? 'border-orange-300 bg-white hover:-translate-y-0.5 hover:shadow-md cursor-pointer'
                                : 'border-dashed border-orange-200 bg-white/70 cursor-default'
                            }`}
                          >
                            ...
                            {!ready && <p className="text-xs text-neutral-500">We're building this next.</p>}
                          </button>
                        );
                      })}
                    </div>
