...
const DemographicsPhase = ({ onComplete, hub }: DemographicsPhaseProps) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 p-4">
        {messages.map((msg, i) => (
          <ChatBubble key={`msg-${i}`} role={msg.role} message={msg.text} hub={hub} />
        ))}
        ...
        {step === 'waitAge' && (
          ...
          >
            <input
              ...
              className={`backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl px-4 py-3 text-white placeholder-white/40 w-32 outline-none transition-colors ${accent.inputBorder}`}
              autoFocus
            />
            <button
              onClick={handleAgeSubmit}
              className={`bg-gradient-to-r ${accent.cta} text-white rounded-2xl px-4 py-3 hover:opacity-90 transition-opacity`}
            >
          ...
          >
            ...
            <div className="flex justify-end">
              <button
                ...
                className={`bg-gradient-to-r ${accent.cta} text-white rounded-2xl px-6 py-3 hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2`}
              >
