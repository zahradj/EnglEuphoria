...
/**
 * Booster pool — when an AI-generated PlaygroundUnit is supplied, narrow
 * the static CORE to words the student met in L1–L3. If nothing overlaps
 * (unit has no matching imagery), fall back to the full CORE so the
 * image-dependent phases below still have art to render.
 */
function useBoosterPool(): Card[] {
  ...
  return useMemo(() => {
    ...
    if (unitWords.size === 0) return CORE;
    const filtered = CORE.filter((c) => unitWords.has(c.word.toLowerCase()));
    return filtered.length >= 3 ? filtered : CORE;
  }, [unit]);
}
...
function ListenPoint() {
  const bank = useBoosterPool();
  const pool = useMemo(() => shuffle(bank).slice(0, Math.min(8, bank.length)), [bank]);
  const [i, setI] = useState(0);
  const [picked, setPicked] = useState<string | null>(null);
  const target = pool[i];
  const choices = useMemo(() => {
    const others = shuffle(bank.filter((c) => c.word !== target.word)).slice(0, 2);
...
function MatchWord() {
  const bank = useBoosterPool();
  const pool = useMemo(() => shuffle(bank).slice(0, Math.min(6, bank.length)), [bank]);
  const [i, setI] = useState(0);
  const [picked, setPicked] = useState<string | null>(null);
  const target = pool[i];
  const choices = useMemo(() => {
