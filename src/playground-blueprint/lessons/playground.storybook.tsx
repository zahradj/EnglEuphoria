...
import {
  ...
  type LookSayLevel,
  type StoryPage,
} from "@/playground-blueprint/lib/storybook";
...
function StorybookLesson() {
  ...
  // AI L4 story → overlay title + scene text onto the static STORY_PAGES.
  // Images, gaps and comprehension stay static (AI doesn't emit those yet).
  const aiStory = unit?.lessons.find((l) => l.lesson_number === 4)?.story;
  const title = aiStory?.title || STORY_TITLE;
  const pages: StoryPage[] = useMemo(() => {
    if (!aiStory?.scenes?.length) return STORY_PAGES;
    return STORY_PAGES.map((base, i) => {
      const scene = aiStory.scenes[i];
      if (!scene?.text) return base;
      // Split AI scene text into sentences for tap-to-hear buttons.
      const sentences = scene.text
        .split(/(?<=[.!?])\s+/)
        .map((s) => s.trim())
        .filter(Boolean);
      return { ...base, text: sentences.length ? sentences : base.text };
    });
  }, [aiStory]);
  ...
  return (
    ...
    >
      {phase === "signed" && <SignedStoryPhase pages={pages} title={title} />}
      {phase === "warmup" && <WarmupPhase />}
      {phase === "read" && <ReadPhase pages={pages} />}
...
function ReadPhase({ pages = STORY_PAGES }: { pages?: StoryPage[] }) {
...
const FALLBACK_PREDICT_OPTIONS = [
...
const EMOJI_BY_WORD: Record<string, string> = {
  cat: "🐱", dog: "🐶", cow: "🐮", pig: "🐷", duck: "🦆",
  tiger: "🐯", monkey: "🐒", snake: "🐍",
  fish: "🐠", octopus: "🐙", crab: "🦀",
};

/** Build prediction options from AI unit vocab + fixed distractors. */
function usePredictOptions(): { label: string; correct: boolean }[] {
  const unit = usePlaygroundUnit();
  return useMemo(() => {
    if (!unit) return FALLBACK_PREDICT_OPTIONS;
    const words = new Set<string>();
    unit.lessons?.forEach((l) => l.vocab?.forEach((w) => words.add(w.toLowerCase().trim())));
    if (!words.size) return FALLBACK_PREDICT_OPTIONS;
    const correct = [...words].map((w) => ({
      label: `${EMOJI_BY_WORD[w] || "✨"} ${/^[aeiou]/.test(w) ? "an" : "a"} ${w}`,
      correct: true,
    }));
...
function WarmupPhase() {
  const PREDICT_OPTIONS = usePredictOptions();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div
          className="flex aspect-square w-full items-center justify-center rounded-3xl text-7xl"
          style={{
            background:
              "linear-gradient(135deg, var(--playground-primary) 0%, var(--playground-accent, #FEFBDD) 100%)",
          }}
          aria-label="Storybook cover"
        >
...
function SequencePhase() {
  ...
  return (
    <div className="space-y-5">
      <TeacherNotes>
        Put the story pages in the right order: jungle first, then ocean. Use the arrows to move a page left or right.
      </TeacherNotes>
      <p className="text-center text-lg font-bold text-[color:var(--playground-ink)]">
        Put the story in order — what happened first?
      </p>
      <div className={STORY_PAGES.length >= 3 ? "grid gap-4 sm:grid-cols-3" : "grid gap-4 sm:grid-cols-2"}>
        {order.map((id, i) => {
...
export default StorybookLesson;
