...
import duckImg from "@/playground-blueprint/assets/animals/duck.png";
import pigImg from "@/playground-blueprint/assets/animals/pig.png";
import petFishImg from "@/playground-blueprint/assets/animals/fish.png";
...
/**
 * Review pool — when an AI-generated PlaygroundUnit is supplied, narrow
 * the static ALL catalog to words the student met in L1–L4. Falls back
 * to the full ALL when no overlap exists so image-driven phases still
 * have art to render. ALL is preserved as an image lookup catalog.
 */
function useReviewPool(): Card[] {
  const unit = usePlaygroundUnit();
  return useMemo(() => {
    if (!unit) return ALL;
    const unitWords = new Set(
      unit.lessons
        .filter((l) => l.lesson_number >= 1 && l.lesson_number <= 4)
        .flatMap((l) => l.vocab ?? [])
        .map((w) => w.toLowerCase()),
    );
    if (unitWords.size === 0) return ALL;
    const filtered = ALL.filter((c) => unitWords.has(c.word.toLowerCase()));
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-2">
        <button
          onClick={() => setI((n) => (n - 1 + pool.length) % pool.length)}
        ...
        <button
          onClick={() => setI((n) => (n + 1) % pool.length)}
          className="glass-card rounded-2xl px-5 py-2 text-sm font-bold"
        >
          Next →
        </button>
...
function SortGame() {
  const reviewPool = useReviewPool();
  const pool = useMemo(() => shuffle(reviewPool).slice(0, 6), [reviewPool]);
...
function Sprint() {
  ...
  const set = useMemo(() => {
    const pool = shuffle(reviewPool).slice(0, Math.min(4, reviewPool.length));
...
function MemoryMatch() {
  const reviewPool = useReviewPool();
  const PAIRS = useMemo(() => shuffle(reviewPool).slice(0, Math.min(6, reviewPool.length)), [reviewPool]);
...
function OddOneOut() {
  ...
  const rounds = useMemo(() => {
    const poolWords = new Set(pool.map((c) => c.word.toLowerCase()));
    // Keep rounds where the "odd" word is in the student's pool so the
    // teaching beat still lands. Colour/size distractors are always allowed.
    const filtered = ODD_ROUNDS.filter((r) => poolWords.has(r.odd.toLowerCase()));
    return filtered.length > 0 ? filtered : ODD_ROUNDS;
  }, [pool]);
  const [i, setI] = useState(0);
  const [score, setScore] = useState(0);
...
function useScrambleRounds(): string[][] {
  const unit = usePlaygroundUnit();
  return useMemo(() => {
    if (!unit) return SCRAMBLE_ROUNDS;
    const sentences = unit.lessons
      .find((l) => l.lesson_number === 4)?.story?.scenes
      ...
      .map((s) => s.split(/\s+/)) ?? [];
    return sentences.length > 0 ? sentences.slice(0, 6) : SCRAMBLE_ROUNDS;
  }, [unit]);
}

function SentenceScramble() {
  const rounds = useScrambleRounds();
  const [round, setRound] = useState(0);
  const target = rounds[round % rounds.length];
  const scrambled = useMemo(() => {
    let s = shuffle(target);
    while (target.length > 1 && s.every((w, i) => w === target[i])) s = shuffle(target);
    return s;
  }, [round, target]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        ...
        {correct && (
          <button
            onClick={() => setRound((r) => (r + 1) % rounds.length)}
      ...
      <p className="text-center text-sm font-bold text-[color:var(--playground-ink)]/60">
        Sentence {round + 1} / {rounds.length}
      </p>
    </div>
  );
}
...
function SpellIt() {
  ...
  const words = useMemo(() => {
    const poolWords = new Set(pool.map((c) => c.word.toLowerCase()));
    const filtered = SPELL_WORDS.filter((w) => poolWords.has(w.toLowerCase()));
    return filtered.length > 0 ? filtered : SPELL_WORDS;
  }, [pool]);
  const [i, setI] = useState(0);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex justify-center gap-2">
        ...
        {correct && (
          <button
            onClick={() => setI((n) => (n + 1) % words.length)}
            className="shadow-playground rounded-2xl px-5 py-2 text-sm font-extrabold text-white"
            style={{ background: "var(--playground-primary)" }}
          >
            Next word →
