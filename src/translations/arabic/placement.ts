export const placementTranslations = {
  'placement.task.listening': 'استمع واختر الإجابة الصحيحة.',
  'placement.task.vocabulary': 'انظر إلى الصورة واختر الكلمة الصحيحة.',
  'placement.task.grammar': 'اختر الكلمة الصحيحة لإكمال الجملة.',
  'placement.task.reading': 'اقرأ بعناية واختر أفضل إجابة.',
  'placement.action.playAudio': 'تشغيل الصوت',
  'placement.action.playAgain': 'إعادة التشغيل',
  'placement.action.loading': 'جارٍ التحميل…',
  'placement.action.playing': 'قيد التشغيل…',
  'placement.action.next': 'السؤال التالي',
  'placement.action.submit': 'إرسال',
  'placement.audio.hint': 'استمع أولاً، ثم اختر إجابتك.',
  'placement.progress.question': 'سؤال',
  'placement.progress.cefr': 'تقييم CEFR',
  'placement.welcome.title.playground': 'مرحبًا بك في الـ Playground، {{name}}!',
  'placement.welcome.title.academy': 'مرحبًا بك في The Academy، {{name}}!',
  'placement.welcome.title.professional': 'مرحبًا بك في Success Hub، {{name}}!',
  'placement.welcome.subtitle': 'قبل فتح لوحتك، دعنا نُقيّم مستواك لتناسب كل درس احتياجاتك تمامًا.',
  'placement.welcome.chip.duration': '≈5 دقائق',
  'placement.welcome.chip.personalized': 'مخصّص لك',
  'placement.welcome.chip.oneTime': 'مرة واحدة فقط',
  'placement.welcome.cta': 'ابدأ تقييمي',
  'placement.welcome.greetingFallback': 'صديقي',
  'placement.choice.title': 'مرحبًا{{comma}} {{name}}! أنا {{mascot}}.',
  'placement.choice.subtitle': 'اختر كيف تريد أن تبدأ:',
  'placement.choice.beginnerTitle': 'أنا مبتدئ تمامًا',
  'placement.choice.beginnerHint': 'تخطَّ الاختبار — سنبدأ من الأساسيات.',
  'placement.choice.testTitle': 'إجراء اختبار تحديد المستوى',
  'placement.choice.testHint': '≈5 دقائق · حتى 15 سؤالًا · يحدد مستواك بدقة.',
  'placement.choice.replay': 'اضغط للاستماع مرة أخرى',
};
