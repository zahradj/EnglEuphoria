export const placementTranslations = {
  'placement.task.listening': 'Escucha y elige la respuesta correcta.',
  'placement.task.vocabulary': 'Mira la imagen y elige la palabra correcta.',
  'placement.task.grammar': 'Elige la palabra correcta para completar la oración.',
  'placement.task.reading': 'Lee con atención y elige la mejor respuesta.',
  'placement.action.playAudio': 'Reproducir audio',
  'placement.action.playAgain': 'Reproducir de nuevo',
  'placement.action.loading': 'Cargando…',
  'placement.action.playing': 'Reproduciendo…',
  'placement.action.next': 'Siguiente pregunta',
  'placement.action.submit': 'Enviar',
  'placement.audio.hint': 'Escucha primero, luego elige tu respuesta.',
  'placement.progress.question': 'Pregunta',
  'placement.progress.cefr': 'Evaluación CEFR',
  'placement.welcome.title.playground': '¡Bienvenido al Playground, {{name}}!',
  'placement.welcome.title.academy': '¡Bienvenido a The Academy, {{name}}!',
  'placement.welcome.title.professional': '¡Bienvenido al Success Hub, {{name}}!',
  'placement.welcome.subtitle': 'Antes de abrir tu panel, vamos a calibrar tu nivel para que cada lección se adapte a ti.',
  'placement.welcome.chip.duration': '≈5 minutos',
  'placement.welcome.chip.personalized': 'Personalizado',
  'placement.welcome.chip.oneTime': 'Solo una vez',
  'placement.welcome.cta': 'Comenzar mi evaluación',
  'placement.welcome.greetingFallback': 'amigo',
  'placement.choice.title': '¡Hola{{comma}} {{name}}! Soy {{mascot}}.',
  'placement.choice.subtitle': 'Elige cómo quieres empezar:',
  'placement.choice.beginnerTitle': 'Soy principiante de verdad',
  'placement.choice.beginnerHint': 'Salta el test — empezaremos desde lo más básico.',
  'placement.choice.testTitle': 'Hacer el test de nivel',
  'placement.choice.testHint': '~5 min · hasta 15 preguntas · encuentra tu nivel exacto.',
  'placement.choice.replay': 'Toca para escuchar de nuevo',
};
