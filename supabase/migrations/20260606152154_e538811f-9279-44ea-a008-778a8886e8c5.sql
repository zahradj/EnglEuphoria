...
ALTER TABLE public.interviews ADD COLUMN IF NOT EXISTS reschedule_count smallint NOT NULL DEFAULT 0;
...
BEGIN
  ...
  IF v_row.status <> 'scheduled' OR v_row.scheduled_at IS NULL THEN
  ...
  SET status = 'awaiting_booking',
      ...
      booking_expires_at = now() + interval '48 hours',
      updated_at = now()
  WHERE id = v_row.id
  RETURNING public.interviews.id,
            ...
            public.interviews.booking_expires_at
