...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  // Send via Resend directly (replaces Lovable email queue).
  ...
  try {
    ...
    if (!resendRes.ok) {
      ...
      console.error('Resend auth send failed', { run_id, emailType, errMsg })
      await supabase.from('email_send_log').insert({
        message_id: messageId,
        template_name: `auth_${emailType}`,
        recipient_email: payload.data.email,
    ...
    await supabase.from('email_send_log').insert({
      ...
      recipient_email: payload.data.email,
      status: 'sent',
      metadata: { resend_id: resendBody?.id, run_id },
    })
    ...
    console.error('Resend auth send threw', { run_id, emailType, errMsg })
    await supabase.from('email_send_log').insert({
      message_id: messageId,
      template_name: `auth_${emailType}`,
      recipient_email: payload.data.email,
